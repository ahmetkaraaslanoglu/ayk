@php use App\Models\Absence; @endphp
<x-app-layout>
            ...
            <div class="flex justify-center">
                <div class="bg-white w-[90%] mt-6">
                    <table class="w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            <th scope="col"
                                class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Öğrenci
                                isimleri
                            </th>
                            <th scope="col"
                        ...
                        @foreach(auth()->user()->school_classes as $school_class)
                            <tbody x-show="id == {{ $school_class->id }}">
                            @foreach($school_class->users()->where('user_school_classes.role','student')->get() as $student)
                                @foreach(Absence::query()->where('owner_id',auth()->id())->where('target_id',$student->id)->get()->groupBy('date') as $date => $absences)
                                    @foreach($absences as $absence)
                                        <tr>
                                            ...
                                            <td class=" py-4 whitespace-nowrap">
                                                <div>{{$absence->date->format('d-m-Y')}} / {{$absence->date->format('h:m')}} </div>
                                            </td>
